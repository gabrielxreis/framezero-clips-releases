...
CURRENT_INSTALLER_VERSION="1.1.21"
...
brand_header() {
  ...
  cat <<TXT
============================================================
          FrameZero Clips Installer 1.1.21 - macOS
============================================================
...
load_manifest() {
  ...
  if ! download_file "$VERSION_URL?cb=$(date +%s)" "$VERSION_FILE" "Consultando atualização..."; then
...
self_update_if_needed() {
  ...
  if [ "$IS_GT" = "1" ]; then
    ...
    if curl -fL "$NEW_URL?cb=$(date +%s)" -o "$NEW_SCRIPT"; then
...
find_python3() {
  for PY in /usr/local/bin/python3 /opt/homebrew/bin/python3 /Library/Frameworks/Python.framework/Versions/Current/bin/python3 "$(command -v python3 2>/dev/null || true)"; do
...
prepare_intel_backend() {
  ...
  if [ -f "$REQ" ]; then
    ...
    if [ -s "$FILTERED_REQ" ]; then "$VPY" -m pip install -r "$FILTERED_REQ" || return 1; fi
  fi
...
download_and_run_clips() {
  ...
  rm -rf "$EXTRACT"
  mkdir -p "$EXTRACT"
  download_file "$MAC_URL?cb=$(date +%s)" "$ZIPPATH" "Baixando FrameZero Clips ${LATEST_VERSION:-}..." || return 1
...
launch_clips() {
  ...
  if [ -f "$LAUNCHER" ]; then chmod +x "$LAUNCHER" 2>/dev/null || true; xattr -d com.apple.quarantine "$LAUNCHER" 2>/dev/null || true; bash "$LAUNCHER"; else open -a "FrameZero Clips" 2>/dev/null || true; fi
}
...
uninstall_clips() {
  ...
  rm -rf "$INSTALL_ROOT" "$HOME/Library/Application Support/FrameZero" "$HOME/Library/Application Support/FrameZero Clips" "$HOME/Library/Caches/FrameZero"* "$HOME/Library/Logs/FrameZero"* "$HOME/Desktop/FrameZero Clips.command" "$HOME/Applications/FrameZero.app" "$HOME/Applications/FrameZero Clips.app" 2>/dev/null || true
